...
collect_latest_gcc_dir() {
  ...
  for root in /usr/lib/gcc /usr/lib64/gcc; do
    [[ -d "$root" ]] || continue
    match="$(find "$root" -mindepth 2 -maxdepth 2 -type d 2>/dev/null | grep '/aarch64[^/]*/' | sort -Vr | head -n 1 || true)"
    if [[ -n "$match" ]]; then
      printf '%s\n' "$match"
      return 0
    fi
...
dynamic_linker="$(find_first_existing /lib/ld-linux-aarch64.so.1 /lib64/ld-linux-aarch64.so.1 /lib/aarch64-linux-gnu/ld-linux-aarch64.so.1 || true)"
crt_entry="$(find_first_existing /usr/lib/aarch64-linux-gnu/Scrt1.o /usr/lib64/Scrt1.o /usr/lib/aarch64-linux-gnu/crt1.o /usr/lib64/crt1.o || true)"
crti_path="$(find_first_existing /usr/lib/aarch64-linux-gnu/crti.o /usr/lib64/crti.o || true)"
crtn_path="$(find_first_existing /usr/lib/aarch64-linux-gnu/crtn.o /usr/lib64/crtn.o || true)"
...
for lib_dir in /usr/lib64 /lib64 /usr/lib /lib /usr/lib/aarch64-linux-gnu /lib/aarch64-linux-gnu; do
  if [[ -d "$lib_dir" ]]; then
