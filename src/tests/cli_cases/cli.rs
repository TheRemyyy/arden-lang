use super::*;
#[cfg(unix)]
use crate::cli::paths::collect_arden_files;
#[cfg(unix)]
...
fn remove_incremental_build_fingerprints(root: &Path) {
    for fingerprint in ["build_fingerprint", "semantic_build_fingerprint"] {
        ...
        if path.exists() {
            fs::remove_file(&path).must("remove incremental build fingerprint");
        }
    }
}
...
fn cli_check_command_succeeds_for_temp_project() {
    ...
    with_current_dir(&temp_root, || {
        check_command(None, false).must("project check should pass");
    });
...
fn cli_build_recovers_from_corrupted_parse_cache_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        let parsed_cache_file = fs::read_dir(&parsed_cache_dir)
            .must("read parse cache dir")
            .filter_map(Result::ok)
            .map(|entry| entry.path())
            .find(|path| path.is_file())
            .must("parse cache file should exist");
        fs::write(&parsed_cache_file, b"not valid cache").must("corrupt parse cache");

        remove_incremental_build_fingerprints(&temp_root);

        build_project(false, false, true, false, false)
            .must("build should ignore corrupted parse cache and recover");
    });
...
fn cli_build_recovers_from_corrupted_import_check_cache_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        let import_cache_file = fs::read_dir(temp_root.join(".ardencache").join("import_check"))
            .must("read import cache dir")
            .filter_map(Result::ok)
            .map(|entry| entry.path())
            .find(|path| path.is_file())
            .must("import cache file should exist");
        fs::write(&import_cache_file, b"not valid cache").must("corrupt import cache");
        remove_incremental_build_fingerprints(&temp_root);

        build_project(false, false, true, false, false)
            .must("build should ignore corrupted import cache and recover");
    });
...
fn cli_build_recovers_from_corrupted_rewrite_cache_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        let rewrite_cache_file = fs::read_dir(temp_root.join(".ardencache").join("rewritten"))
            .must("read rewrite cache dir")
            .filter_map(Result::ok)
            .map(|entry| entry.path())
            .find(|path| path.is_file())
            .must("rewrite cache file should exist");
        fs::write(&rewrite_cache_file, b"not valid cache").must("corrupt rewrite cache");
        remove_incremental_build_fingerprints(&temp_root);

        build_project(false, false, true, false, false)
            .must("build should ignore corrupted rewrite cache and recover");
    });
...
fn cli_build_recovers_from_corrupted_object_cache_metadata_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        let object_cache_file = fs::read_dir(temp_root.join(".ardencache").join("objects"))
            .must("read object cache dir")
            .filter_map(Result::ok)
            .map(|entry| entry.path())
            .find(|path| path.extension().and_then(|ext| ext.to_str()) == Some("json"))
            .must("object cache metadata should exist");
        fs::write(&object_cache_file, b"not valid cache").must("corrupt object cache meta");
        remove_incremental_build_fingerprints(&temp_root);

        build_project(false, false, true, false, false)
            .must("build should ignore corrupted object cache metadata and recover");
    });
...
fn cli_build_recovers_from_corrupted_dependency_graph_cache_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        fs::write(&dependency_graph_cache, b"not valid cache")
            .must("corrupt dependency graph cache");
        remove_incremental_build_fingerprints(&temp_root);

        build_project(false, false, true, false, false)
            .must("build should ignore corrupted dependency graph cache and recover");
    });
...
fn cli_build_recovers_from_corrupted_semantic_summary_cache_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        build_project(false, false, true, false, false)
            .must("build should ignore corrupted semantic summary cache and recover");
    });
...
fn cli_build_recovers_from_corrupted_typecheck_summary_cache_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        fs::write(&typecheck_summary_cache, b"not valid cache")
            .must("corrupt typecheck summary cache");
        remove_incremental_build_fingerprints(&temp_root);

        build_project(false, false, true, false, false)
            .must("build should ignore corrupted typecheck summary cache and recover");
    });
...
fn cli_build_recovers_from_corrupted_link_manifest_cache_blob() {
    ...
    with_current_dir(&temp_root, || {
        ...
        let link_manifest_cache = temp_root
            ...
            .join("link")
            .join("latest.json");
        fs::write(&link_manifest_cache, b"not valid cache").must("corrupt link manifest cache");
        remove_incremental_build_fingerprints(&temp_root);

        build_project(false, false, true, false, false)
            .must("build should ignore corrupted link manifest cache and recover");
    });
...
fn cli_format_targets_checks_and_formats_project_files() {
    ...
    .must("write unformatted file");

    with_current_dir(&temp_root, || {
        let err = format_targets(None, true).must_err("format check should fail before fmt");
        assert!(err.contains("format check failed"), "{err}");
        format_targets(None, false).must("format should succeed");
        format_targets(None, true).must("format check should pass after fmt");
    });

    let formatted = fs::read_to_string(&main_file).must("read formatted file");
    assert!(
...
fn cli_run_tests_lists_filtered_tests_in_directory() {
    ...
    run_tests(Some(&temp_root), true, Some("smoke")).must("test listing should succeed");
...
fn cli_run_tests_recurses_into_nested_test_directories() {
    ...
    fs::create_dir_all(&nested_dir).must("create nested test dir");
    ...
    .must("write nested test file");

    run_tests(Some(&temp_root.join("tests")), true, Some("nested"))
        .must("nested directory test listing should succeed");
...
fn cli_run_tests_discovers_mixed_case_test_filenames() {
    ...
    .must("write mixed-case test file");

    run_tests(Some(&temp_root), true, Some("mixedCase"))
        .must("mixed-case test discovery should succeed");
...
fn cli_run_tests_errors_for_missing_directory() {
    ...
    let err =
        run_tests(Some(&missing_dir), true, None).must_err("missing test directory should error");
    assert!(
...
fn cli_run_tests_rejects_non_arden_file_paths() {
    ...
    .must("write non-arden file");

    let err = run_tests(Some(&text_file), true, None).must_err("non-arden file path should fail");
    assert!(
...
fn cli_run_tests_reports_source_context_for_parse_errors() {
    ...
    .must("write malformed test source");

    let err = run_tests(Some(&test_file), true, None)
        .must_err("test command should report parse source context");
...
fn cli_run_tests_without_path_uses_project_file_list_only() {
    ...
    fs::create_dir_all(&examples_dir).must("create examples dir");
    write_test_project_config(
        &temp_root,
    ...
    .must("write listed test");
    fs::write(
        examples_dir.join("broken_test.arden"),
        "@Test\nfunction broken(: None { return None; }\n",
    )
    .must("write stray broken test");

    with_current_dir(&temp_root, || {
        run_tests(None, true, Some("listedTest"))
            .must("default project test discovery should ignore non-project files");
    });
...
fn cli_run_tests_without_path_executes_tests_in_non_test_named_project_files() {
    ...
    fs::write(
        ...
        .must("write main with test");

    with_current_dir(&temp_root, || {
        run_tests(None, false, Some("smokeFromMain"))
            .must("project default test discovery should execute tests from non-test-named files");
    });
...
fn cli_run_tests_does_not_delete_existing_test_runner_neighbor_files() {
    ...
    .must("write test file");
    fs::write(&existing_runner, "keep me\n").must("write neighboring runner file");
    fs::write(&existing_exe, "keep me too\n").must("write neighboring exe file");

    run_tests(Some(&test_file), false, Some("smoke"))
        .must("test execution should succeed without touching neighboring files");

    assert_eq!(
        fs::read_to_string(&existing_runner).must("read neighboring runner"),
        "keep me\n"
    );
    assert_eq!(
        fs::read_to_string(&existing_exe).must("read neighboring exe"),
        "keep me too\n"
    );
...
fn cli_run_tests_executes_project_local_alias_import_tests() {
    ...
    with_current_dir(&temp_root, || {
        run_tests(None, false, Some("aliasImportTest"))
            .must("project-local alias imports in tests should run successfully");
    });
...
fn cli_run_tests_accepts_relative_project_file_path() {
    ...
    fs::write(
        ...
        .must("write main test file");

    with_current_dir(&temp_root, || {
        run_tests(Some(Path::new("src/main.arden")), false, Some("smoke"))
            .must("relative project file path should execute tests");
    });
...
fn cli_run_tests_accepts_relative_single_file_path_in_current_directory() {
    ...
    with_current_dir(&temp_root, || {
        run_tests(Some(Path::new("smoke_test.arden")), false, Some("smoke"))
            .must("relative single-file path should execute tests");
    });
...
fn cli_run_tests_skips_before_hooks_for_ignored_tests() {
    ...
    .must("write ignored before test");

    run_tests(Some(&test_file), false, Some("skipped"))
        .must("ignored test should skip before hook execution");
...
fn cli_run_tests_skips_after_hooks_for_ignored_tests() {
    ...
    .must("write ignored after test");

    run_tests(Some(&test_file), false, Some("skipped"))
        .must("ignored test should skip after hook execution");
...
fn cli_run_tests_accepts_ignore_reasons_with_literal_braces() {
    ...
    .must("write ignored braces test");

    run_tests(Some(&test_file), false, Some("skipped"))
        .must("ignored test reason with literal braces should execute cleanly");
...
fn cli_run_tests_skips_symlinked_directories() {
    ...
    let outside_dir = temp_root.parent().must("temp root parent").join(format!(
        "arden-outside-tests-{}-{}",
        std::process::id(),
        SystemTime::now()
    ...
    fs::create_dir_all(&outside_dir).must("create outside dir");
    fs::write(
        outside_dir.join("escape_test.arden"),
        "@Test\nfunction escaped(): None { return None; }\n",
    )
    .must("write outside test");
    symlink(&outside_dir, tests_dir.join("linked-outside")).must("create dir symlink");

    let files = find_test_files(&tests_dir).must("test discovery should skip symlink directories");
    assert!(
        files.is_empty(),
...
fn cli_run_tests_rejects_symlinked_file_paths_outside_root() {
    ...
    let outside_file = temp_root.parent().must("temp root parent").join(format!(
    ...
    .must("write outside test file");
    symlink(&outside_file, &linked_file).must("create file symlink");

    let err = run_tests(Some(&linked_file), true, None)
        .must_err("test command should reject symlinked files escaping the root");
    assert!(
...
fn cli_run_tests_accepts_symlinked_file_paths_inside_root() {
    ...
    .must("write real test file");
    symlink(&real_file, &linked_file).must("create file symlink");

    run_tests(Some(&linked_file), true, Some("smoke"))
        .must("safe in-tree symlinked test file should be accepted");
...
fn cli_run_tests_rejects_paths_through_symlinked_directories() {
    ...
    .must("write outside test");
    symlink(&outside_dir, &linked_dir).must("create dir symlink");

    let err = run_tests(Some(&linked_file), true, None)
        .must_err("test command should reject symlinked ancestor directories");
    assert!(
...
fn cli_format_targets_rejects_project_files_outside_root() {
    ...
    with_current_dir(&temp_root, || {
        let err =
            format_targets(None, true).must_err("fmt should reject project files outside the root");
        assert!(
...
fn collect_arden_files_rejects_symlinked_file_paths_outside_root() {
    ...
    let err = collect_arden_files(&linked_file)
        .must_err("fmt file collection should reject symlinked files escaping the root");
    assert!(
        err.contains("resolves outside the requested directory tree"),
        "{err}"
    );
...
fn collect_arden_files_accepts_symlinked_file_paths_inside_root() {
    ...
    fs::write(&real_file, "function smoke(): None { return None; }\n")
        .must("write real arden file");
    symlink(&real_file, &linked_file).must("create file symlink");

    let files = collect_arden_files(&linked_file)
        .must("safe in-tree symlinked arden file should be accepted");
...
fn collect_arden_files_rejects_paths_through_symlinked_directories() {
    ...
    let err = collect_arden_files(&linked_file)
        .must_err("fmt file collection should reject symlinked ancestor directories");
    assert!(
        err.contains("must not traverse symlinked directories"),
        "{err}"
    );
...
fn cli_lint_target_rejects_entry_outside_root() {
    ...
    with_current_dir(&temp_root, || {
        let err = lint_target(None).must_err("lint should reject entry outside the root");
        assert!(
            err.contains("resolves outside the project root"),
            "expected outside-root validation error, got: {err}"
        );
...
fn cli_lint_target_rejects_directory_paths() {
    ...
    let err = lint_target(Some(&temp_root)).must_err("lint should reject directory paths");
    assert!(
...
fn cli_info_rejects_invalid_project_opt_level() {
    ...
    with_current_dir(&temp_root, || {
        let err = show_project_info().must_err("info should reject invalid opt level");
...
fn cli_info_rejects_non_arden_entry_path() {
    ...
    with_current_dir(&temp_root, || {
        let err = show_project_info().must_err("info should reject non-arden entry");
        assert!(
...
fn cli_info_rejects_directory_entry_path() {
    ...
    with_current_dir(&temp_root, || {
        let err = show_project_info().must_err("info should reject directory entry path");
        assert!(
...
fn cli_info_rejects_non_arden_secondary_file_path() {
    ...
    with_current_dir(&temp_root, || {
        let err = show_project_info().must_err("info should reject non-arden secondary file");
        assert!(
            err.contains("must resolve to an .arden source file")
                || err.contains("is not an .arden file"),
            "{err}"
...
fn cli_fix_target_rejects_non_arden_file_paths() {
    ...
    let text_file = temp_root.join("notes.txt");
    fs::write(&text_file, "not arden\n").must("write text file");

    let err = fix_target(Some(&text_file)).must_err("fix should reject non-arden files");
    assert!(
        err.contains("is not an .arden file"),
...
fn cli_lex_file_rejects_directory_paths() {
    ...
    let err = lex_file(&temp_root).must_err("lex should reject directory paths");
    assert!(
        err.contains("is not a file"),
        "expected directory path validation error, got: {err}"
    );
...
fn cli_parse_file_rejects_directory_paths() {
    ...
    let err = parse_file(&temp_root).must_err("parse should reject directory paths");
    assert!(
        err.contains("is not a file"),
        "expected directory path validation error, got: {err}"
    );
...
fn cli_parse_file_reports_source_context_for_parse_errors() {
    ...
    fs::write(&source_file, "function main(: None { return None; }\n")
        .must("write malformed source");

    let err = parse_file(&source_file).must_err("parse should report syntax error");
...
fn cli_compile_file_rejects_non_arden_paths() {
    ...
    let err = compile_file(&text_file, None, false, true, None, None)
        .must_err("compile should reject non-arden files");
    assert!(
        err.contains("is not an .arden file"),
        "expected non-arden validation error, got: {err}"
    );
...
fn cli_commands_consistently_reject_invalid_files_before_output_processing() {
    ...
    let err = compile_file(&text_file, Some(&output_path), false, true, None, None)
        .must_err("compile should reject invalid source before touching output path");
    ...
    assert!(
        !output_path.parent().must("parent").exists(),
...
fn cli_project_commands_consistently_report_invalid_file_list_entries() {
    ...
    with_current_dir(&temp_root, || {
        let info_err = show_project_info().must_err("info should reject invalid files list entry");
        let check_err = check_file(None).must_err("check should reject invalid files list entry");
        let lint_err = lint_target(None).must_err("lint should reject invalid files list entry");
...
fn cli_compile_creates_missing_output_parent_directory() {
    ...
    compile_file(&source_file, Some(&output_path), true, true, None, None)
        .must("compile should create missing output directories");
...
fn cli_check_file_rejects_directory_paths() {
    ...
    let err = check_file(Some(&temp_root)).must_err("check should reject directory paths");
    assert!(
        err.contains("is not a file"),
        "expected directory path validation error, got: {err}"
    );
...
fn cli_check_file_rejects_non_arden_paths() {
    ...
    let err = check_file(Some(&text_file)).must_err("check should reject non-arden files");
    assert!(
        err.contains("is not an .arden file"),
        "expected non-arden validation error, got: {err}"
    );
...
fn cli_check_command_reports_cross_file_type_errors() {
    ...
    with_current_dir(&temp_root, || {
        let err = check_command(None, false).must_err("project check should fail");
        assert!(
...
fn cli_info_rejects_output_path_matching_project_config() {
    ...
    with_current_dir(&temp_root, || {
        let err =
            show_project_info().must_err("info should reject output path matching project config");
