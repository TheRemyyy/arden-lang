...
- Fixed module-local shadowing of exact builtin import aliases:
  - project rewrite now extends nested module import maps with the module's own `import` declarations before doing module-local call/type rewriting and then alpha-renames locals and lambda params that shadow exact imported aliases such as `import Option.None as Empty`
  - this fixes valid project builds such as `module Inner { import Option.None as Empty; function keep(): Option<Integer> { Empty: Option<Integer> = Option.Some(7); return Empty; } }`, `async { Empty: Integer = 7; Empty }`, and `f: () -> Integer = || Empty`, which previously resolved the shadowed local back to the builtin alias and either returned `None` or failed with follow-on type mismatches
...
- Fixed `Task.await_timeout(...)` coercion for exact zero-argument integer aliases:
  - task timeout validation and codegen now route the millisecond argument through the same contextual zero-argument alias coercion path used by other integer argument boundaries
  - this fixes valid project builds such as `work().await_timeout(ArgCount)`, which previously failed with `Undefined variable: Args__count`
...
- Fixed operator-expression coercion for exact zero-argument aliases:
  - unary and binary operator type-checking/codegen now treat exact zero-argument aliases as concrete operand values instead of unresolved raw builtin symbols
  - this fixes valid project builds such as `Pi > 3.14 && Pi < 3.15` and `value: Float = -Pi`, which previously failed with repeated `Undefined variable: Math__pi` diagnostics
- Fixed method-receiver coercion for exact zero-argument aliases:
  - method-call receiver inference now treats exact zero-argument aliases as concrete values instead of unresolved canonical symbols before dispatching builtin or class methods
  - this fixes valid project builds such as `CurrentDir.length()`, which previously failed with `Undefined variable: std__system__cwd`
- Fixed builtin exact import aliases as method receivers:
  - `Option.None` and similar builtin exact import aliases now materialize their receiver values during project rewrite instead of being treated as unresolved static owners
  - this fixes valid project builds such as `import Option.None as Empty; Empty.is_none();`, which previously failed with `Undefined variable: Option__None`
- Fixed control-flow receiver coercion for exact zero-argument aliases:
  - builtin-aware receiver inference now propagates concrete zero-argument alias types through `if` and block expressions before method dispatch and code generation
  - this fixes valid project builds such as `(if (true) { CurrentDir } else { CurrentDir }).length()` and `({ CurrentDir }).length()`, which previously failed with `Undefined variable: System__cwd`
- Fixed match-expression receiver coercion for exact zero-argument aliases:
  - match arm tail inference and code generation now preserve concrete zero-argument alias values instead of collapsing them to unresolved builtin symbols or `None`
  - this fixes valid project builds such as `(match (true) { true => CurrentDir, false => CurrentDir, }).length()`, which previously failed with `Undefined variable: System__cwd`
