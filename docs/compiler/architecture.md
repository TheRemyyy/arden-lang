...
That object-expression inference now also covers expression-valued `if`/`match` results and block tails, so direct field/method chains on those objects survive lowering instead of failing late in codegen. Parser-side constructor heuristics now also recognize explicit generic constructors through postfix parsing paths, which keeps `Boxed<Integer>(...)` constructor syntax stable even inside nested expression branches. The parser now also treats the final statement inside `if`-expression branches as an expression-valued tail when it reaches the closing brace, including bare `match (...) { ... }` forms, so branch typing stays aligned with the AST the typechecker expects instead of degrading valid expression branches into statement-only `None` paths. Equality lowering now also reuses expected `Option`/`Result` container types when one comparison operand is a static tagged constructor like `Result.ok(...)` or `Result.error(...)`, which keeps nested tagged comparisons ABI-consistent instead of falling back to placeholder constructor inference during backend codegen. Match-expression lowering now uses the surrounding expected result type for tagged tail arms as well, so branches such as `Error(_) => Option.none()` emit the same nested container layout as sibling `Ok(value) => value` arms instead of producing structurally incompatible LLVM values. The same expected-type plumbing now also applies to `if`-expression tail arms, so `Option.none()` / `Result.error(...)` branches stay ABI-consistent with sibling tagged-constructor branches instead of producing non-dominating placeholder IR values. Nested tagged container equality is now also null-safe for inactive string payload slots, preventing grown `Set<Result<..., String>>` / `Map<Result<..., String>, ...>` paths from dereferencing null error pointers during comparisons of non-error variants. Project-mode filtered codegen now also emits local class methods whenever the owning class itself is active, preventing shadowed local receivers like `u: Local = Local(...); u.get()` from dropping required local method bodies during object-file emission.
